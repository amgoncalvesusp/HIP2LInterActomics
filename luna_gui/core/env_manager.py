...
def miniconda_download_url() -> str:
    ...
    if sys.platform == "darwin":
        ...
        if machine not in architectures:
            raise RuntimeError(f"Arquitetura macOS não suportada automaticamente: {machine}")
    ...
    if machine not in architectures:
        raise RuntimeError(f"Arquitetura Linux não suportada automaticamente: {machine}")
    arch = architectures[machine]
