...
def _app_root() -> Path:
    """Directory holding the importable ``luna_gui`` package on disk.

    The external luna-env interpreter runs the FP-dashboard helper via
    ``python -c`` and imports ``luna_gui.core.results_analysis`` from this
    path. In a normal checkout that is the repo root two levels up. When the
    GUI is frozen by PyInstaller the source tree is shipped as data under the
    bundle root (``sys._MEIPASS``), so the path that the *external* interpreter
    can read must come from there, not from ``__file__`` (which then points
    inside the PYZ archive and is not importable on disk).
    """
...
def _helper_failure(
    result: subprocess.CompletedProcess, prefix: str = "Helper falhou"
) -> dict:
    try:
    ...
    detail_parts = [
        part
        for part in (
            _clean_helper_text(result.stderr),
            _clean_helper_text(result.stdout),
        )
        if part
    ]
    ...
    if not detail:
        detail = (
            "sem saida de erro relevante; apenas avisos conhecidos foram filtrados."
        )
...
def _build_shell_level_only_fp_detail(
    wd: Path, ifp_type: str, output_path: Path
) -> dict | None:
    ...
    """
    suffix = {value: key for key, value in IFP_SUFFIX_TO_TYPE.items()}.get(
        str(ifp_type).upper()
    )
    if suffix is None:
        return None
        ...
        for row in rows or []:
            ...
            for raw_level, raw_count in breakdown.items():
                ...
                try:
                    ...
                    count = 1
                detail["shell_level_counts"][level] = (
                    detail["shell_level_counts"].get(level, 0) + count
                )
                entry_info["shell_level_counts"][level] = (
                    entry_info["shell_level_counts"].get(level, 0) + count
                )
...
def _residue_matrix_has_ligand_atoms(artifact: dict | None) -> bool:
    if not isinstance(artifact, dict):
        return False
    return bool(artifact.get("ligand_atoms")) and isinstance(
        artifact.get("ligand_atom_matrix"), dict
    )
...
def run_residue_matrix(
    ...
    if cached is not None and (
        not require_ligand_atoms or _residue_matrix_has_ligand_atoms(cached)
    ):
        return cached
    ...
    for candidate in iter_pymol_candidates(py_exe):
        try:
            if (
                ...
                and _path_is_inside(candidate, prefix)
            ):
                return candidate
        except OSError:
            continue
...
def generate_fp_session(
    ...
    if result.returncode != 0:
        prefix = (
            "Helper PyMOL/LUNA falhou"
            if pymol_exe is not None
            else "Helper LUNA/PyMOL falhou"
        )
...
def run_fp_detail_analysis(
    ...
    shell_level_only = _build_shell_level_only_fp_detail(
        wd, str(ifp_type).upper(), output_path
    )
...
def run_fp_dashboard_analysis(
    ...
    try:
        ...
        return {
            "error": "A geração do dashboard de fingerprints excedeu o tempo limite."
        }
    except Exception as exc:
        return {"error": str(exc)}
